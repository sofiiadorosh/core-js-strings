...
function sumOfCodes(str) {
  ...
  return [...str].reduce((acc, char) => acc + char.charCodeAt(), 0);
}
      ...
      word
        ...
        .map((letter) => {
          const index = alphabet
            .split('')
            .findIndex((char) => char === letter.toLowerCase());
          if (index === -1) {
            return letter;
          }

          let encodedIndex = index + shift;
          if (encodedIndex > alphabet.length - 1) {
            encodedIndex -= alphabet.length;
          }
          return letter === letter.toLowerCase()
            ? alphabet[encodedIndex].toLowerCase()
            : alphabet[encodedIndex].toUpperCase();
        })
        .join('')
    )
    .join(' ');
