...
function countVowels(str) {
  return (str.toLowerCase().match(/[aeiouy]/g) || []).length;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (char) => {
    ...
    if (code <= UPPERCASE_END) {
      return String.fromCharCode(
        ((code - UPPERCASE_START + ROTATION) % ALPHABET_SIZE) + UPPERCASE_START
      );
    }

    return String.fromCharCode(
      ((code - LOWERCASE_START + ROTATION) % ALPHABET_SIZE) + LOWERCASE_START
    );
  });
}
